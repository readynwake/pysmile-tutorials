...
class Tutorial1:
    ...
    #Create Conditional Probablitynodes table(CPT) node
    #net(net), id("Economy"), name("State of economy"), outcomes(string array),
    #x_pos and y_pos mean position on the screen
    def create_cpt_node(self, net, id, name, outcomes, x_pos, y_pos):
        #Add a CPT type node with id
        handle = net.add_node(pysmile.NodeType.CPT, id)

        #Set name, position of node
        net.set_node_name(handle, name)
        net.set_node_position(handle, x_pos, y_pos, 85, 55)

        #Setting outcome into node ?
